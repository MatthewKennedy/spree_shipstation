...
RSpec.describe(Spree::Shipment) do
  describe ".between" do
    ...
    it("returns shipments whose order updated_at falls within the given time range") do
    ...
    it("does not return shipments whose order updated_at falls outside the given time range") do
      order = create(:order) { |o| o.update_column(:updated_at, Time.zone.now) }
